...
def retrieval_precision(label_dist_arr, label_arr, k):
    ...
    topk = topk.numpy()
    topk_labels = labels[topk][:k]
    # topk = np.apply_along_axis(lambda x: label_dict[x+1], axis=1, arr=topk)
    masked_topk = np.array([np.empty(k).fill(label_arr[i])] for i in range(topk.shape[0]))
    breakpoint()
    masked_topk = label_arr == topk_labels
    
    return np.array([precision_at_k(mask.squeeze(), k) for mask in masked_topk])
